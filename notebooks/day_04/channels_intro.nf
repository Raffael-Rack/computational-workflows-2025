...
workflow{
    ...
    if (params.step == 3) {
        out_ch = channel.fromPath("/workspaces/computational-workflows-2025/notebooks/day_04/files_dir/*")
    }

    // Task 4 - Create a channel that includes all TXT files in the "files_dir" directory
    
    if (params.step == 4) {
        out_ch = channel.fromPath("/workspaces/computational-workflows-2025/notebooks/day_04/files_dir/*.txt")
    }

    // Task 5 - Create a channel that includes the files "fastq_1.fq" and "fastq_2.fq" in the "files_dir" directory

    if (params.step == 5) {
        out_ch = channel.fromPath("/workspaces/computational-workflows-2025/notebooks/day_04/files_dir/fastq_*.fq")    
    }
    ...
    if (params.step == 6) {
        out_ch = channel.fromPath("/workspaces/computational-workflows-2025/notebooks/day_04/files_dir/*", hidden: true)
    }

    // Task 7 - get all filepairs in the "files_dir" directory

    if (params.step == 7) {
        out_ch = channel.fromFilePairs("/workspaces/computational-workflows-2025/notebooks/day_04/files_dir/*_{1,2}.*", hidden: true)
    }
